...
def main():
    ...
    # ============================================================
    # ============================================================
    # SECTION 6: V20.6.1 GRADUAL RISK LAYER VALIDATION
    # ============================================================
    print("\n" + "=" * 70)
    print("SECTION 6: V20.6.1 GRADUAL RISK LAYER VALIDATION")
    print("=" * 70)
    ...
    # DD_TRIGGER removed — replaced by gradual DD_FLOOR in Component 4
    DD_FLOOR = -0.50           # max acceptable DD for gradual scaling
    FEE = 0.0005
    SLIPPAGE = 0.0005
    TOTAL_FRICTION = FEE + SLIPPAGE

    print(f"  Config: target_vol={TARGET_VOL}, cap={POSITION_CAP}, DD_floor={DD_FLOOR}")
    ...
    for fold_i, fd in enumerate(fold_results):
        ...
        # Component 4: GRADUAL DD scalar (replaces binary breaker)
        DD_FLOOR = -0.50  # max acceptable DD
        ...
        for t in range(len(scale_final)):
            ...
            # Gradual scalar: linearly reduce exposure as DD deepens
            dd_scalar = np.clip(1.0 + dd / abs(DD_FLOOR), 0.1, 1.0)
            scale_final[t] = scale_final[t] * dd_scalar

            if dd_scalar < 1.0:
                dd_reduced_bars += 1

            # Update equity AFTER applying scale
        ...
        risk_layer_results.append({
            ...
            'dd_reduced_bars': dd_reduced_bars,
        ...
        print(f"    DD_reduced:    {dd_reduced_bars} bars active, mean_scale={np.mean(scale_final):.3f}")
    ...
    # ============================================================
    # SECTION 7: V20.6.1 FINAL VERDICT
    # ============================================================
    print("\n" + "=" * 70)
    print("SECTION 7: V20.6.1 FINAL VERDICT")
    print("=" * 70)
    ...
    print(f"\n  --- PER-FOLD COMPARISON ---")
    print(f"  {'Fold':<6} {'Sh_raw':<10} {'Sh_net':<10} {'DD_raw':<10} {'DD_net':<10} {'DD_reduced':<10}")
    for fi, r in enumerate(risk_layer_results):
        print(f"  {fi:<6} {r['sharpe_raw']:<10.3f} {r['sharpe_net']:<10.3f} {r['max_dd_raw']:<10.2%} {r['max_dd_net']:<10.2%} {r['dd_reduced_bars']:<10d}")
    ...
    # Telegram
    tg_msg = f"""<b>V20.6.1 Gradual Risk Layer Validation</b>
